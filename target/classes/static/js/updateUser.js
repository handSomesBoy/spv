...
function sendMessage(o,flag){
    ...
    if (wait == 60) {
        ...
        //自定义验证规则
        $.post("/sys/sendMessage", {"mobile":$("#telephone").val(),"picCode":$("#picCode").val()}, function (data) {
...
function updatePwd(){
    ...
    if(flag!=false){
        ...
        $.post("/sys/updatePwd",{"mobile":$("#telephone").val(),"picCode":$("#picCode").val(),"mobileCode":mobileCode},function(data){
            console.log("data:"+data)
            if(data.code=="1000"){
                layer.closeAll();
                layer.open({
...
function setPwd(){
    ...
    $.post("/sys/setPwd",{"pwd":pwd,"isPwd":isPwd},function(data){
        console.log("data:"+data);
        if(data.code=="1000"){
