...
$(function(){
    ...
    console.info(baseURL+"/spUnit/queryUnitList");
    $("#jqGrid").jqGrid({
        url:baseURL+"spUnit/queryUnitList",
        datatype: "json",
        mtype: 'POST',
        height:window.screen.height-550,
        colModel: [
            { label: 'id', name: 'unitId', width: 75,hidden:true},
            { label: '单位名称', name: 'unitName', width: 200 },
            { label: '单位编码', name: 'unitCode', width: 200 },
            { label: '备注', name: 'remark', width: 200,formatter: function(cellvalue, options, cell){
                return '<a  href="'+cell.url+'" class="btn btn-purple btn-sm" target="_blank"><i class="fa fa-cog  fa-spin" aria-hidden="true"></i>"'+cellvalue+'"点我</a>';
            }}
        ],
        pager: "#jqGridPager",
        rowNum:10,
    ...
    methods: {
        ...
        update:function(){
            ...
            if(rows==0){
                ...
                vm.title="修改教程";
                vm.unit = $("#jqGrid").jqGrid('getRowData', rows[0]);
                $("#addModal").modal({
                    keyboard : false,
                    show : true,
                    backdrop : "static"
        ...
        saveOrUpdate:function(){
            ...
            var url = vm.unit.unitId == null ? "spUnit/add" : "spUnit/update";
            $.ajax({
                type: "POST",
                url: baseURL+url,
        ...
        reload: function () {
            ...
            $("#jqGrid").jqGrid('setGridParam',{
                postData:{'condition':{unitName:vm.qryUnitName}},
                page:page
            }).trigger("reloadGrid");
        }
    }
