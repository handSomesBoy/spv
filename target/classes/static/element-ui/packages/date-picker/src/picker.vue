...
export default {
  ...
  methods: {
    ...
    // {parse, formatTo} String deals with testUser input
    parseString(value) {
    ...
    handleKeydown(event) {
      ...
      if (keyCode === 9) {
        if (!this.ranged) {
          ...
          // testUser may change focus between two input
          setTimeout(() => {
      ...
      // if testUser is typing, do not let picker handle key input
      if (this.userInput) {
        event.stopPropagation();
        return;
      }
    ...
    emitChange(val) {
      // determine testUser real change only
