...
- Pagination
  - Fixed `current-change` event wrongly triggering without testUser interaction, #10247
- DatePicker
...
- Fixed Select not triggering validation when testUser selects an option after executing `resetFields` of Form, #10105
...
- Table
  ...
  - Fixed layout not update when column width is changed by testUser dragging, #9668
...
- General
  ...
  - `change` event of form components and `current-change` event of Pagination now only trigger on testUser interaction
...
- Input
  ...
  - `change` event now behaves like the native input element, which triggers only on blur or pressing enter. If you need to respond to testUser input in real time, you can use `input` event.
- Autocomplete
