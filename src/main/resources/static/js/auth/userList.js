...
function setJobUser(obj,id,nameVersion,checked){
    ...
    layer.confirm('您确定要把用户：'+name+'设置为'+userIsJob+'状态吗？', {
        ...
        $.post("/sys/setJobUser",{"id":id,"job":isJob,"version":version},function(data){
            if(isLogin(data)){
                if(data=="ok"){
                    //回调弹框
...
//开通用户
function addUser(){
    $.get("/sys/getRoles",function(data){
        if(isLogin(data)){
            if(data!=null){
                //显示角色数据
                $("#roleDiv").empty();
...
function getUserAndRoles(obj,id) {
    ...
    if(obj.job){
        ...
        //回显数据
        $.get("/sys/getUserAndRoles",{"id":id},function(data){
            if(isLogin(data)){
    ...
    if(null!=id){
        if(currentUser==id){
            ...
            layer.confirm('您确定要删除'+name+'用户吗？', {
                ...
                $.post("/sys/delUser",{"id":id,"version":version},function(data){
                    if(isLogin(data)){
                        if(data=="ok"){
                            //回调弹框
...
function recoverUser(obj,id) {
    ...
    if(null!=id){
        layer.confirm('您确定要恢复'+name+'用户吗？', {
            ...
            $.post("/sys/recoverUser",{"id":id,"version":version},function(data){
                if(isLogin(data)){
                    if(data=="ok"){
                        //回调弹框
