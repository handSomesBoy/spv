...
import com.linkspring.spv.config.filter.KickoutSessionFilter;
import com.linkspring.spv.config.shiro.RetryLimitHashedCredentialsMatcher;
...
;		logger.debug("-----------------Shiro拦截器工厂类注入开始");
		...
		//配置记住我或认证通过可以访问的地址
		/*filterChainDefinitionMap.put("/sys/userList", "sys");
...
//		// 配置不会被拦截的链接 从上向下顺序判断
		...
		filterChainDefinitionMap.put("/toLogin?kickout=1", "anon");
		filterChainDefinitionMap.put("/login", "anon");
		filterChainDefinitionMap.put("/loginCheck", "anon");
		filterChainDefinitionMap.put("/unauthorized", "anon");
		//filterChainDefinitionMap.put("/sys/delUser", "authc,perms[usermanage]");//需要经过授权 待改造
		//启动过程中不能调用通用mapper的相关dao
		List<Permission> permissionLists= sysPermissionService.getAllPermission();
		String page="";
		String pageCode="";
		if(!permissionLists.isEmpty())
		{
			for(Permission permission : permissionLists)
			{
				...
				pageCode=permission.getCode();
				if(StringUtils.isNotEmpty(page)&&StringUtils.isNotEmpty(pageCode)){
					filterChainDefinitionMap.put("/"+permission.getPage(), "perms["+permission.getCode()+"]");
				}
			}
		}
//		// <!-- authc:所有url都必须认证通过才可以访问; anon:所有url都都可以匿名访问【放行】-->
		filterChainDefinitionMap.put("/home", "kickout,authc");//
		filterChainDefinitionMap.put("/mainPage", "kickout,authc");//
		filterChainDefinitionMap.put("/sysMenu", "kickout,authc");//
		filterChainDefinitionMap.put("/spv/**","kickout,authc");
