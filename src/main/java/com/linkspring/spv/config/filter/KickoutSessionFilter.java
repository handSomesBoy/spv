package com.linkspring.spv.config.filter;
...
import com.linkspring.spv.utils.ShiroFilterUtils;
...
public class KickoutSessionFilter extends AccessControlFilter {
	...
	protected boolean onAccessDenied(ServletRequest request,
		...
		if (!subject.isAuthenticated() && !subject.isRemembered()) {
			...
			if (ShiroFilterUtils.isAjax(request) ) {
				...
				responseResult.setMessage("您已在离线，请重新登录");
	...
	public static void out(ServletResponse response, ResponseResult result){
		...
		try {
			...
			logger.error("用户在线数量限制【spv-manager-->KickoutSessionFilter.out】响应json信息成功");
		} catch (Exception e) {
			logger.error("用户在线数量限制【spv-manager-->KickoutSessionFilter.out】响应json信息出错", e);
		}finally{
			if(null != out){
				out.flush();
				out.close();
