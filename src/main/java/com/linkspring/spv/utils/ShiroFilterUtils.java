...
public class ShiroFilterUtils {
    ...
    public static boolean isAjax(ServletRequest request){
    	...
    	if("XMLHttpRequest".equalsIgnoreCase(header)){
    		logger.debug("shiro工具类【spv-manager-->ShiroFilterUtils.isAjax】当前请求,为Ajax请求");
    		return Boolean.TRUE;
    	}
    	logger.debug("shiro工具类【spv-manager-->ShiroFilterUtils.isAjax】当前请求,非Ajax请求");
	...
	public static void out(HttpServletResponse response, ResponseResult result){
		...
		try {
			...
			logger.error("用户在线数量限制【spv-spv-->ShiroFilterUtils.out】响应json信息成功");
		} catch (Exception e) {
			logger.error("用户在线数量限制【spv-spv-->ShiroFilterUtils.out】响应json信息出错", e);
		}finally{
			if(null != out){
				out.flush();
				out.close();
